...
class OfferController extends BaseController
{
    public function index()
    {
        $offers = DB::table('offers as of')
            ...
            ->where('status_id', '<>', 3)
            ->select('of.user_id', 'of.created_at', 'of.name', 'of.description', 'of.slug', 'of.id', 'p.name as project_name', 'status.id as status', 'status.name as status_name', 'u.name as username')
