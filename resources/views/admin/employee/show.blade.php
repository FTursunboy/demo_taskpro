...
@section('content')
    ...
    <div id="page-heading">
        ...
        @if(empty($user->getRoleNames()[1]))
            <a role="button" class="btn btn-outline-success mb-2 mx-2" data-bs-toggle="modal" data-bs-target="#role">
                            ...
                            <div class="col-6">
                                <div class="d-flex justify-content-end">
                                    ...
                                    <button class="btn btn-outline-primary" data-bs-toggle="modal"
                                            data-bs-target="#addUser"><i class="bi bi-person-plus mx-3"></i> Добавить
                                    ...
                                    <div class="modal fade" id="addUser" data-bs-backdrop="static"
                                        ...
                                        <div class="modal-dialog modal-lg">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="addUser">Выберите сотрудник</h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                            aria-label="Close"></button>
                                                </div>
                                                <form action="{{ route('employee.add-user-in-command', $user->id) }}"
                                                      method="POST">
                                                    @csrf
                                                    <div class="modal-body">
                                                        <div class="form-group mt-4">
                                ...
                                @foreach($project_tasks as $project_task)
                                    <tr>
                                        ...
                                        @switch($project_task->status->id)
                                            ...
                                            <td><span class="badge bg-warning">{{ $project_task->status->name }}</span>
                                            </td>
                                            @break
                                            @case($project_task->status->id === 2)
                                            <td><span class="badge bg-success">{{ $project_task->status->name }}</span>
                                            </td>
                                            @break
                                            ...
                                            <td><span class="badge bg-primary">{{ $project_task->status->name }}</span>
                                            </td>
                                            @break
                                            @case($project_task->status->id === 5)
                                            <td><span class="badge bg-danger">{{ $project_task->status->name }}</span>
                                            </td>
                                            @break
                                            @case($project_task->status->id === 6)
                                            <td>
                                                <span class="badge bg-light-info">{{ $project_task->status->name }}</span>
                                            </td>
                                            @break
                                            @case($project_task->status->id === 7)
                                            <td>
                                                <span class="badge bg-secondary">{{ $project_task->status->name }}</span>
                                            </td>
                                            @break
