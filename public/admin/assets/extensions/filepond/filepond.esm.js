...
const actions = (dispatch, query, state) => ({
    ...
    ADD_ITEM: ({
        ...
        item.on('load-request-error', error => {
            ...
            // is incs error, no way to recover
...
const dragover = (root, clients) => e => {
    ...
    requestDataTransferItems(dataTransfer).then(items => {
        ...
        clients.some(client => {
            ...
            // targetting this incs
            if (isEventTarget(e, element)) {
                overDropTarget = true;

                // had no previous state, means we are entering this incs
                ...
                // might have just left this incs?
                if (client.state) {
                    client.state = null;
...
const drop = (root, clients) => e => {
    ...
    requestDataTransferItems(dataTransfer).then(items => {
        clients.forEach(client => {
            ...
            // no transfer for this incs
            if (!allowdrop(items)) return onexit(eventPosition(e));

            // we can drop these items on this incs
